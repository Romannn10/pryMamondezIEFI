    ...
    internal class ClsClientes
    {
            ...
            try
            {
                ...
                if (DR.HasRows)
                {
                    ...
                    while (DR.Read())
                    {
                        ...
                        //Añade filas a la grilla tomando las posiciones de los campos de la tabla Principal
        ...
        public void GenerarReporte(PrintPageEventArgs Reporte)
        {
            try
            {
                ...
                Reporte.Graphics.DrawString("Direccion", FuenteSubTitulo, Brushes.Blue, 350, Linea - 15);
                Reporte.Graphics.DrawString("Barrio", FuenteSubTitulo, Brushes.Blue, 450, Linea - 15);
                Reporte.Graphics.DrawString("Actividad", FuenteSubTitulo, Brushes.Blue, 550, Linea - 15);
                Reporte.Graphics.DrawString("Saldo", FuenteSubTitulo, Brushes.Blue, 650, Linea - 15);
                Reporte.Graphics.DrawString("Edad", FuenteSubTitulo, Brushes.Blue, 750, Linea - 15);
                ...
                if (Data.Tables[Tabla].Rows.Count > 0) //Si tiene filas
                {
                    ...
                    foreach (DataRow fila in Data.Tables[Tabla].Rows)
                    {
                        ...
                        Reporte.Graphics.DrawString(fila["Direccion"].ToString(), FuenteTexto, Brushes.Black, 350, Linea);
                        Reporte.Graphics.DrawString(fila["Saldo"].ToString(), FuenteTexto, Brushes.Black, 650, Linea);
                        Reporte.Graphics.DrawString(fila["Edad"].ToString(), FuenteTexto, Brushes.Black, 750, Linea);
                        Linea = Linea + 15;
                    }

                    while (Lector.Read())
                    {
                        NombreACT = Actividad.Buscar(Lector.GetInt32(3));
                        NombreBR = Barrio.Buscar(Lector.GetInt32(4));
                        Reporte.Graphics.DrawString(NombreBR, FuenteTexto, Brushes.Black, 450, Linea -45);
                        Reporte.Graphics.DrawString(NombreACT, FuenteTexto, Brushes.Black, 550, Linea-45);
                        Linea = Linea + 15;
                    }
